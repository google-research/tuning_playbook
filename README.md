...
- [深度学习调优指南中文版](#深度学习调优指南中文版)
  ...
  - [开始新项目的指南](#开始新项目的指南)
    ...
    - [选择BatchSize](#选择batchsize)
      - [确定可行的Batch Size并估计训练吞吐量](#确定可行的batch-size并估计训练吞吐量)
      - [选择Batch Size以最小化训练时间](#选择batch-size以最小化训练时间)
      - [更改Batch Size需要重新调整大多数超参数](#更改batch-size需要重新调整大多数超参数)
      - [Batch Norm会对Batch Size的选择造成什么影响？](#batch-norm会对batch-size的选择造成什么影响)
      - [选择初始配置](#选择初始配置)
  - [提高模型性能的科学方法](#提高模型性能的科学方法)
    ...
    - [设计下一轮实验](#设计下一轮实验)
      - [识别目标超参数、冗余超参数和固定超参数](#识别目标超参数冗余超参数和固定超参数)
      - [创建一组研究](#创建一组研究)
      - [平衡实验的信息量和成本](#平衡实验的信息量和成本)
    - [从实验结果中获取经验](#从实验结果中获取经验)
      - [识别错误的搜索空间边界](#识别错误的搜索空间边界)
      - [没有在搜索空间中采样足够的点](#没有在搜索空间中采样足够的点)
      - [检查训练曲线](#检查训练曲线)
      - [使用isolation图检测更改是否有用](#使用isolation图检测更改是否有用)
      - [自动化常用的绘图](#自动化常用的绘图)
    - [确定是否采用此训练流水线更改或超参数配置](#确定是否采用此训练流水线更改或超参数配置)
    - [探索结束后](#探索结束后)
  - [确定每次训练运行的步数](#确定每次训练运行的步数)
    - [当训练不受计算限制时如何决定该训练多久](#当训练不受计算限制时如何决定该训练多久)
      - [使用学习率搜索算法来确定 max\_train\_steps 的初始值](#使用学习率搜索算法来确定-max_train_steps-的初始值)
  ...
  - [关于训练流水线的额外补充](#关于训练流水线的额外补充)
    ...
    - [评估模型性能](#评估模型性能)
      - [评估设置](#评估设置)
      - [设置定期评估](#设置定期评估)
      - [选择样本进行定期评估](#选择样本进行定期评估)
    - [保存检查点并追溯选择最佳检查点](#保存检查点并追溯选择最佳检查点)
    - [设置实验跟踪](#设置实验跟踪)
    - [BatchNorm的实现细节](#batchnorm的实现细节)
    - [多主机流水线的注意事项](#多主机流水线的注意事项)
  - [常见问题的回答](#常见问题的回答)
    - [为什么将学习率和其他优化参数称为超参数？ 它们不是任何先验分布的参数。](#为什么将学习率和其他优化参数称为超参数-它们不是任何先验分布的参数)
    - [为什么不应该调整Batch Size来直接提高验证集性能?](#为什么不应该调整batch-size来直接提高验证集性能)
    - [所有流行的优化算法的更新规则是什么？](#所有流行的优化算法的更新规则是什么)
